...
public class TiDBQueryBuilder {
  ...
  public TiDBQueryBuilder(String quote) {
  ...
  public PreparedStatement buildSql(
    ...
    // append tidb split
    sql.append(String.format(" TABLESPLIT('%s', '%s') ", startKey, endKey));
    ...
    if (additionalPredicate.isPresent()) {
      clauses =
          ImmutableList.<String>builder()
              ...
              .build();
      accumulator.addAll(
          additionalPredicate.get().getBoundConstantValues().stream()
              .map(
                  constantExpression ->
                      new TypeAndValue(constantExpression.getType(), constantExpression.getValue()))
    ...
    for (int i = 0; i < accumulator.size(); i++) {
      ...
      if (typeAndValue.getType().equals(BigintType.BIGINT)) {
        ...
        statement.setDate(
            i + 1, new Date(UTC.getMillisKeepLocal(DateTimeZone.getDefault(), millis)));
      ...
      } else if (typeAndValue
          .getType()
          .equals(TimestampWithTimeZoneType.TIMESTAMP_WITH_TIME_ZONE)) {
        statement.setTimestamp(
            i + 1, new Timestamp(unpackMillisUtc((long) typeAndValue.getValue())));
  ...
  private String toPredicate(
      String columnName, Domain domain, Type type, List<TypeAndValue> accumulator) {
    ...
    for (Range range : domain.getValues().getRanges().getOrderedRanges()) {
      ...
      if (range.isSingleValue()) {
        ...
        // If rangeConjuncts is null, then the range was ALL, which should already have been checked
        // for
  ...
  private String toPredicate(
      String columnName, String operator, Object value, Type type, List<TypeAndValue> accumulator) {
    bindValue(value, type, accumulator);
