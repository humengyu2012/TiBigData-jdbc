...
public abstract class CanalDeserializationSchema implements DeserializationSchema<RowData> {
  ...
  protected CanalDeserializationSchema(
    ...
    Preconditions.checkArgument(
        TiDBJdbcUtils.getLogicalTs(startTs) == 0, "The logical ts must be 0 for canal format");
  ...
  public void deserialize(FlatMessage flatMessage, Collector<RowData> out) {
    ...
    long version = TiDBJdbcUtils.getVersion(flatMessage.getEs(), 0L);
    if (startTs > version) {
      return;
    }
    ...
    switch (type) {
      ...
      case INSERT:
        data.stream()
            .map(values -> toRowData(version, values, RowKind.INSERT))
            .forEach(out::collect);
        break;
      case DELETE:
        data.stream()
            .map(values -> toRowData(version, values, RowKind.DELETE))
            .forEach(out::collect);
        break;
      case UPDATE:
        old.stream()
            .map(values -> toRowData(version, values, RowKind.UPDATE_BEFORE))
            .forEach(out::collect);
        data.stream()
            .map(values -> toRowData(version, values, RowKind.UPDATE_AFTER))
            .forEach(out::collect);
        break;
      default:
        if (!ignoreParseErrors) {
  ...
  protected RowData toRowData(long version, Map<String, String> values, RowKind rowKind) {
    // ignore case
    values =
    ...
    for (int i = 0; i < metadata.length; i++) {
      ...
      rowData.setField(fieldNames.size() + i, tiDBMetadata.extract(version));
    }
    return rowData;
  }
