...
public class TiDBSourceBuilder implements Serializable {
  ...
  public TiDBSourceBuilder(
      ResolvedCatalogTable table, TiDBSchemaAdapter schema, String whereCondition, Integer limit) {
    this.schema = schema;
  ...
  private long getStartingOffsetTs(long version) {
    return Optional.ofNullable(properties.get(TiDBOptions.STARTING_OFFSETS_TS))
        .filter(StringUtils::isNotEmpty)
        .map(Long::parseLong)
        .orElse(TiDBJdbcUtils.getPhysicalTs(version) - TiDBOptions.MINUTE);
  }

  private CDCSourceBuilder<?, ?> createCDCBuilder(long version) {
    if (streamingSource.equals(STREAMING_SOURCE_KAFKA)) {
      return CDCSourceBuilder.kafka(
              databaseName, tableName, version, schema, getStartingOffsetTs(version))
  ...
  public Source<RowData, ?, ?> build() {
    final SnapshotSource source =
        new SnapshotSource(databaseName, tableName, properties, schema, whereCondition, limit);
            ...
            context -> {
              {
                ...
                if (previousEnumerator == null) {
                  ...
                  version = previousEnumerator.getVersion();
                }
                final CDCSourceBuilder<?, ?> cdcBuilder = createCDCBuilder(version);
